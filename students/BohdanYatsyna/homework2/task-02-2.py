...
with open("../tasks/02-structural-linguistics/data/examiner-headlines.txt") as file:
    data = file.readlines()
statistic = {'entity':0, 'most lvl':0, 'polarity':0}
for line in tqdm(data):
    ...
    for token in doc:
        ...
        if token.ent_type_ == "PESRON" or token.ent_type_ == "ORG" or token.ent_type_ == "GPE"\
...
with open('../students/BohdanYatsyna/homework2/task-02-2-statistic.txt', 'w') as f:
    print('examiner-headlines.txt - statistic entities: {:2.2%} most lvl: {:2.2%} polarity: {:2.2%}'.format(statistic['entity']/5000,\
            statistic['most lvl']/5000,statistic['polarity']/5000),file=f)